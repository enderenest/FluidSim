...
void Fluid::Update(float dt) {
	...
	_spatialLookup.bindTo(5);
	_startIndices.bindTo(6);

    _updateShader.setFloat("dt", dt);
    _updateShader.setFloat("gravity", _gravityAcceleration);
	_updateShader.setFloat("mass", _mass);
	_updateShader.setFloat("collisionDamping", _collisionDamping);
	_updateShader.setFloat("smoothingRadius", _smoothingRadius);
	_updateShader.setFloat("targetDensity", _targetDensity);
	_updateShader.setFloat("pressureMultiplier", _pressureMultiplier);
	_updateShader.setFloat("viscosityStrength", _viscosityStrength);
	_updateShader.setFloat("nearDensityMultiplier", _nearDensityMultiplier);

    _updateShader.dispatch((_particleCount + 255) / 256);
    _updateShader.wait();
}
